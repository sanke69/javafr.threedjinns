...
public class ObjectCenterController extends GxCamera3DController {
	private double speed = 1.0;

	public ObjectCenterController(Node _viewport, GxCamera3D _camera) {
	...
	protected Optional<EventHandler<KeyEvent>>   keyEventHandler() {
		return Optional.of((evt) -> {
			...
			switch(evt.getCode()) {
			case Z: 		this.camera.raise(- step);
							break;
			case S: 		this.camera.raise(step);
							break;
			case Q: 		this.camera.strafe(- step);
							break;
			case D: 		this.camera.strafe(step);
							break;
			case R: 		this.camera.move(step);
							break;
			case F: 		this.camera.move(- step);
							break;
			...
			camera.lookAt(Points.of(0, 0, 0), Vectors.of(0, 0, 1));
		});
	}
	...
	protected Optional<EventHandler<ScrollEvent>> scrollEventHandler() {
		return Optional.of((ScrollEvent t) -> {
			if(camera == null)
				return ;

			if(t.getDeltaY() > 0)
				this.camera.move(100);
			else
				this.camera.move(- 100);
		});
